...
**NARROW-TAPE MOOD RULE (ratified 2026-09-22 — brief-review ledger I-10).**
When §7 classifies breadth as NARROW, the Market Mood may NOT be a bare
"Neutral/mixed." It must either state an index-specific lean ("mixed tape,
SPY-bearish tilt") or explicitly claim a pin — the mood label has to use the
brief's own breadth diagnosis. This exists because all four lifetime mood
misses (9/2, 9/4, 9/8, 9/16) were the identical shape: a neutral label on a
self-declared NARROW morning where SPY then trended past the ±0.25% band —
...
**CARRY-FORWARD CATALYST CALENDAR (ratified 2026-09-22 — brief-review ledger
I-13).** Maintain a persistent list of dated binaries — PDUFA/AdComm dates,
earnings, lockup expiries, index-inclusion effective dates — sourced from prior
briefs, day-plan cards, and review open items (readable from the scoring DB's
`open_items`). Each run, check that carried list ALONGSIDE the fresh
API/`fda-calendar` pull. A fresh pull may ADD a date but may NEVER silently
DELETE or downgrade a previously recorded one: removing a catalyst from the
watch requires an explicit retraction with a source, stated in the brief. This
exists because on 9/17 a fresh (self-flagged low-confidence) calendar pull
wrote RARE's UX111 PDUFA "not near-term," contradicting our own 9/14 record of
a 9/19 date — and RARE ran +12.6% pre-binary, unwatched. Our own files knew;
the fresh pull silently overrode them.
...
**ACCUMULATION ESCALATION (ratified 2026-08-25 — brief-review ledger I-2).**
A watchlist name whose flow/OI has leaned the SAME direction in **≥3
consecutive briefs** is escalated to FLAGGED — even with a flat premarket
print and no news — and the flag quantifies the cumulative build (contracts,
premium, strikes across the streak), not just today's slice. A same-morning
snapshot cannot see a position being built patiently; only memory can. This
exists because HIMS burned the quiet list five times in five graded reviews
...
trigger and its status — and may not soften or downgrade the call on flow
evidence alone. Flow may add color ("the put build was not pressed"); only a
*price* development (the trigger un-confirming, the invalidation firing) may
change the call. This is playbook §0 — price action overrules flow — applied
to the brief's own follow-through. It exists because on 8/19 WDC was marked
"cooling" (the $500-put OI barely grew) one session after the 8/18 card's
"loses $500" trigger had confirmed at the close; the price answer was a
rejection at exactly $500.00 and a −6.9% day. The same flow-lean-over-price
error has recurred on the flag side three times (NVDA 8/19, MRK 8/20,
PLTR 8/24).

**REPEAT-MOVER RULE (ratified 2026-09-22 — brief-review ledger I-12).** A
watchlist name with **≥2 unflagged ≥3% close-to-close moves within the trailing
10 sessions** may not take a bare "nothing notable" line — it carries a
two-sided level line (above X / below Y) until it has been quiet for five
consecutive sessions. This is the mechanical floor for the leanless-mention
leak, enforceable from the scoring DB exactly like the fragility floor (I-4).
It exists because the mentioned-but-leanless class was the largest remaining
recall leak across the graded reviews — RDDT ×4, HIMS ×6, TE ×3, IBRX ×2 among
others — names quiet-lined into a ≥3% move the same day.

**DATED CORPORATE EVENTS (ratified 2026-09-22 — brief-review ledger I-9,
scoped).** Every FLAGGED watchlist name and every large-cap name carries any
dated NON-earnings company event inside the next 5 sessions — product launches,
analyst/investor days, lockup expiries, index-inclusion effective dates —
sourced, with the same discipline as the earnings calendar. **Scope (owner
ruling 2026-09-22):** apply only to flagged + large-cap names, and only to
events that surface in the news/UW passes the brief already runs — do NOT build
a new all-watchlist sweep. It exists because TSLA (8/31, +5.5%) was quiet-lined
ahead of its publicly-dated Cybercab event and RDDT (9/2, +9.3%) moved on
S&P-inclusion news the run missed.

**MUST-MENTION CHECKLIST (added 2026-08-25, supports I-2/I-4/I-7).** Each run,
read `brief-review/MUST_MENTION.md` from this repo (it is regenerated by the
review process from the scoring database after each graded session). Every
name listed there must appear in §6A with at least the required sentence for
its listed reason — the fragility floor, the active-complex note, or the
accumulation flag. If the file is stale (its data-through date is more than
one trading day behind), say so in §13 and apply the three rules from this
spec by hand. The checklist is a floor, not a ceiling.
...
**FIRST-ELIGIBLE-CANDLE RULE (ratified 2026-09-22 — brief-review ledger I-14).**
Every confirmation states the first candle that may satisfy it, and NO
confirmation may be satisfied by the 9:30–9:45 opening candle or by an opening
gap print — the trigger clock starts at 9:45 unless the card explicitly says
otherwise. This mirrors §0's own "first 10–15 min: watch, don't trade" and the
day-plan event protocol, which the radar template did not previously inherit.
It exists because on 9/18 both radar failures confirmed on the first candle and
reversed (QQQ 718 long, USO 156.67 up-line); the guard's first live day (9/21)
produced three first-candle refusals and three paid confirms.

**GAP RULE FOR TRIGGERS (ratified 2026-09-22 — brief-review ledger I-8).** A
trigger level that price OPENS beyond is void until price re-crosses it, and
when premarket sits within ~1% of a written trigger the card must state what
the trigger means if gapped past (stand down / retest-only / re-anchored level)
— not merely that it is void. Prefer closing-state triggers ("15-min close
above X"), which survive gaps, over event "break of X" triggers, which are
gap-fragile. Complements I-14 (which governs levels satisfied AT the open). It
exists because on 9/16 XLE gapped 74¢ through its trigger and the
protocol-correct entry paid 1.10 for a spread priced at 0.55 premarket and lost
20.9% with the direction right all session; TSLA 9/3's closing-state trigger
was gap-robust and paid +5.4%.

**CARD-QA TIMING RULE (ratified 2026-09-22 — brief-review ledger I-11).** Every
card that names a catalyst states the catalyst's datetime, and the card's
confirmation/invalidation may only resolve at moments AFTER it. A card whose
written test would resolve before its own catalyst is defective by construction
and must be caught at write time. It exists because the 9/9 ORCL volatility
card wired its test to "Thursday's open" while ORCL reported Thursday after the
close — the test would have graded itself before the event existed.
