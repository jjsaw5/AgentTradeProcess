...
| `/api/stock/{t}/greek-exposure` | verified 2026-08-21: daily series (251 rows) of aggregate `call_charm`/`put_charm`/`call_vanna`/`put_vanna`/gamma/delta — vendor-computed dealer-exposure greeks by day |
| `/api/stock/{t}/greek-exposure/strike` | verified 2026-08-21: same greeks **per strike** (202 strikes on TSLA). Charm concentration by strike locates where time-decay hedge flow lives — on 8/21 OPEX it sat overhead in the 360–400 calls, positive below spot in the 330–340 put book |
| `/api/stock/{t}/spot-exposures` | verified 2026-08-21: **intraday per-minute** series (316 rows by 12:43 ET) with `price` and `charm/gamma/vanna_per_one_percent_move` in `_oi`/`_vol`/`_dir` weightings. The live charm gauge: OI-weighted charm deepened −597B→−655B from morning to midday on OPEX, i.e. the time-decay engine measurably strengthens intraday. **Sign convention (whose book, which direction) is UNVERIFIED** — the `_dir` and `_oi` variants carried opposite signs on 8/21, so use magnitude/trend/strike-structure, and reason hedge direction mechanically from moneyness (ITM-call and dying-put books) rather than trusting the raw sign. `spot-exposures/strike` also answers (50 rows, bid/ask-split fields) |
